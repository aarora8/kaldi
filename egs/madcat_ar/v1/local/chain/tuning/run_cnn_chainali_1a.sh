...
affix=_1a.semisup  #affix for TDNN+LSTM directory e.g. "1a" or "1b", in case we change the configuration.
chain_model_dir=exp/chain${nnet3_affix}/cnn_e2eali_1b.semisup
common_egs_dir=
reporting_email=
...
lat_dir=exp/chain${nnet3_affix}/e2e_${train_set}_lats_chain.semisup
ali_dir=exp/chain/chain_ali_train.semisup
dir=exp/chain${nnet3_affix}/cnn_chainali${affix}
train_data_dir=data/${train_set}
tree_dir=exp/chain${nnet3_affix}/tree_chainali.semisup
...
if [ $stage -le 4 ]; then
  ...
  relu-batchnorm-dropout-layer name=tdnn4 input=Append(-4,0,4) dim=$tdnn_dim dropout-proportion=0.0
  relu-batchnorm-layer name=prefinal-chain dim=$tdnn_dim target-rms=0.5 $tdnn_opts
  output-layer name=output include-log-softmax=false dim=$num_targets max-change=1.5 $output_opts
  relu-batchnorm-layer name=prefinal-xent input=tdnn4 dim=$tdnn_dim target-rms=0.5 $tdnn_opts
