...
if [ $stage -le 5 ]; then
  steps/nnet3/decode_semisup.sh --num-threads 4 --nj $nj --cmd "$cmd" --beam 15 \
...
lattice_prune_beam=0.0  # beam for pruning the lattices prior to getting egs
...
if [ $stage -le 18 ]; then
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --frames-per-chunk 340 --nj $nj --cmd "$cmd" \
