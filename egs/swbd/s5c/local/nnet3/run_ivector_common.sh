...
if [ $stage -le 3 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $mfccdir/storage ]; then
    ...
    utils/create_split_dir.pl /export/b{03,04,09,02}/$USER/kaldi-data/mfcc/swbd-$date/s5b/$mfccdir/storage $mfccdir/storage
  fi

  utils/copy_data_dir.sh data/train_nodup_sp data/train_nodup_sp_hires

  utils/data/perturb_data_dir_volume.sh data/train_nodup_sp_hires

  steps/make_mfcc.sh --nj 30 --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" data/train_nodup_sp_hires exp/make_hires/train_nodup_sp $mfccdir;
  steps/compute_cmvn_stats.sh data/train_nodup_sp_hires exp/make_hires/train_nodup_sp $mfccdir;

  utils/fix_data_dir.sh data/train_nodup_sp_hires;
